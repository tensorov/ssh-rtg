...
### Option B: Ansible (recommended for multi-host setups)
...
│   ├── inventories/
│   │   └── sample/                  # Example inventory (copy and modify)
│   │       ├── hosts.yml
│   │       └── group_vars/all.yml
│   ├── playbooks/
│   │   ├── deploy-client.yml        # Deploy tunnel client on NAT hosts
│   │   └── deploy-server.yml        # Deploy tunnel server on VPS
│   └── roles/
│       ├── ssh-tunnel-client/       # Client role (NAT side)
│       │   ├── defaults/main.yml    # All configurable variables
│       │   ├── tasks/main.yml       # User, key, script, service setup
│       │   ├── tasks/config.yml     # Config summary file
│       │   ├── templates/
│       │   │   ├── tunnel.sh.j2     # Tunnel client script (auto-reconnect)
│       │   │   └── ssh-tunnel.service.j2  # Systemd unit file
│       │   └── handlers/main.yml    # Restart handler
│       └── ssh-tunnel-server/       # Server role (VPS side)
│           ├── defaults/main.yml    # All configurable variables
│           ├── tasks/main.yml       # Traefik routes, firewall, sysctl, user
│           ├── tasks/firewall.yml   # UFW / nftables port rules
│           ├── templates/
│           │   ├── traefik-tcp-routes.yml.j2  # Traefik dynamic TCP config
│           │   └── firewall-rules.sh.j2       # Firewall rule generator
│           └── handlers/main.yml    # Traefik/ufw restart handlers
├── client-script/
│   ├── config.env                   # Configuration template
│   ├── tunnel.sh                    # Standalone tunnel client
│   └── install.sh                   # Systemd installer (coming soon)
├── LICENSE                          # MIT
├── README.md
├── .ansible-lint
└── .yamllint
...
### Client Role Variables (`ssh-tunnel-client`)
...
### Server Role Variables (`ssh-tunnel-server`)
...
Before running the server playbook, the VPS must have:
